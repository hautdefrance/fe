# PWA

- [下一代 Web 应用模型 —— Progressive Web App](https://huangxuan.me/2017/02/09/nextgen-web-pwa/)
- [饿了么的 PWA 升级实践](https://huangxuan.me/2017/07/12/upgrading-eleme-to-pwa/)
- [改造你的网站，变身 PWA](https://segmentfault.com/a/1190000008880637)
- [【Service Worker】生命周期那些事儿](https://75team.com/post/lifecycle.html)
- [ServiceWorker](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorker)
- [Service Worker 更新机制](https://harttle.land/2017/04/10/service-worker-update.html)
- [Service Worker 全面进阶](https://www.villainhr.com/page/2017/01/08/Service%20Worker%20%E5%85%A8%E9%9D%A2%E8%BF%9B%E9%98%B6)
- [Progressive Web Apps Training](https://developers.google.com/web/ilt/pwa/)
- [使用 Service Workers](https://developer.mozilla.org/zh-CN/docs/Web/API/Service_Worker_API/Using_Service_Workers)
- [ServiceWorker](https://w3c.github.io/ServiceWorker/#serviceworker-interface)

## API

- [skipWaiting](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerGlobalScope/skipWaiting)
- [waitUntil](https://developer.mozilla.org/en-US/docs/Web/API/ExtendableEvent/waitUntil)
- [CacheStorage.open()](https://developer.mozilla.org/en-US/docs/Web/API/CacheStorage/open)
- [onupdatefound](https://developer.mozilla.org/zh-CN/docs/Web/API/ServiceWorkerRegistration/onupdatefound)
- [Clients.claim()](https://developer.mozilla.org/en-US/docs/Web/API/Clients/claim)


## Awesome PWA

- [workbox](https://github.com/GoogleChrome/workbox)
